...
static t_fract	ft_fract_burning(t_fract fract, int x, int y, t_env *env)
{
...
void			ft_burning(t_env *env)
{
	...
	while (fract.x < X_SIZE)
	{
		...
		{
			fract = ft_fract_burning(fract, fract.x, fract.y, env);
			fract.i = 0;
					...
					&& (fract.i < IT_MAX))
			{
				...
				fract.z_r = ((fract.z_r * fract.z_r) - (fract.z_i * fract.z_i)
					+ fract.c_r);
				fract.z_i = ((2 * fabs(fract.z_i * fract.tmp)) + fract.c_i);
				fract.i++;
			}
